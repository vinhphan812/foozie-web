...
StoreSchema.static({
     createCart: async function (userId) {
          return this.create({ _id: userId, data: {} });
     },
     addCart: async function (userId, food_id, quantity) {
          ...
          if (store.data[food_id]) {
               if (quantity !== 0) {
                    store.data[food_id] = store.data[food_id] + quantity;
               }
          } else {
               store.data[food_id] = quantity;
          }
          // if (store.data[food_id]) {
